const BACKEND_URL = (
  ...
  ? 'http://localhost:3000/' 
  : 'https://geraldine-edwards-quote-generator-backend.hosting.codeyourfuture.io/';
...
async function addQuote(quote, author) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      messageElem.textContent = error || 'Could not add your quote. Please check your input and try again.';
    ...
    if (!messageElem) return;
    messageElem.textContent = 'Network error: Unable to reach the server. Please try again later.';
